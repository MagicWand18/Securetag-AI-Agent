      ...
      >
        <div
          className={cn("transition-all duration-300", {
            "bg-white/90 dark:bg-slate-950/90 border-slate-200 dark:border-slate-800 mx-4 rounded-full border pr-2 shadow-lg backdrop-blur-md md:mx-20 lg:pr-0":
              isScrolled,
            "bg-white/80 dark:bg-slate-950/80 border-slate-200 dark:border-slate-800 mx-0 border-b backdrop-blur-md":
              !isScrolled,
          })}
        >
          <nav
          ...
          >
            <div className="flex items-center gap-6">
              ...
              {centerContent ? (
                <div className="ml-4 hidden lg:block">{centerContent}</div>
              ) : (
    ...
    <div className="hidden items-center justify-end gap-3 lg:flex lg:flex-1">
      ...
      {isUserLoading ? null : !user ? (
        <WaspRouterLink
          ...
          className={cn(
            "ml-3 font-mono font-semibold uppercase tracking-wide leading-6 transition-all duration-300",
            {
              "text-sm": !isScrolled,
              "text-xs": isScrolled,
            },
        ...
        >
          <div className="text-slate-300 hover:text-blue-400 flex items-center transition-colors duration-300 ease-in-out">
            Log in{" "}
            <LogIn
          ...
          <SheetHeader>
            <SheetTitle className="flex items-center">
              <WaspRouterLink to={routes.LandingPageRoute.to}>
                <span className="sr-only">SecureTag.Agent</span>
...
const NavLogo = ({ isScrolled }: { isScrolled: boolean }) => (
  <div
    className={cn(
      "transition-all duration-500 rounded-full overflow-hidden flex items-center justify-center",
      {
        "size-8": !isScrolled,
        "size-7": isScrolled,
      },
