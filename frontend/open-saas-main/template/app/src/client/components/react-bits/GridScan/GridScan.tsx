...
type GridScanProps = {
  ...
  // Deprecated/Unused in this simplified version but kept for compatibility if needed
...
function smoothDampVec2(
  ...
  if ((originalTo.x - current.x > 0) === (output_x > originalTo.x)) {
    output_x = originalTo.x;
    currentVelocity.x = (output_x - originalTo.x) / deltaTime;
  }
  if ((originalTo.y - current.y > 0) === (output_y > originalTo.y)) {
    output_y = originalTo.y;
    currentVelocity.y = (output_y - originalTo.y) / deltaTime;
  }

  return new THREE.Vector2(output_x, output_y);
}

function smoothDampFloat(
  current: number,
  target: number,
  currentVelocityState: { v: number },
  smoothTime: number,
  maxSpeed: number,
  deltaTime: number
  ...
  const temp = (currentVelocityState.v + omega * change) * deltaTime;
  currentVelocityState.v = (currentVelocityState.v - omega * temp) * exp;

  let output = target + (change + temp) * exp;

  if ((originalTo - current > 0) === (output > originalTo)) {
    output = originalTo;
    currentVelocityState.v = (output - originalTo) / deltaTime;
  }

  return { value: output, v: currentVelocityState.v };
}
