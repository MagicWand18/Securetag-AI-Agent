...
curl -X POST -H "x-api-key: spartaneai-token-2025" -F "file=@qa_artifacts/vuln_test.zip" -F "project_alias=spartane-test-local" http://localhost/codeaudit/upload
...
curl -X POST -H "x-api-key: spartaneai-token-2025" -F "file=@test/frontend.zip" -F "project_alias=my-first-scan" -F "custom_rules=true" -F "custom_rules_qty=1" -F "double_check=all" -F "double_check_level=standard" -F "profile=auto" https://api.securetag.com.mx/codeaudit/upload
