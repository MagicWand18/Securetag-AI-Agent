...
| **Frontend** | ✅ **Completado** | ⏸️ **Standby** | Dashboard, Billing & Identity implementados. |
| **FullStack** | 🟢 **Activo** | ✅ **SI** | Ejecución de Backlog (Hotfixes & Features). |
...
| **Fase 14: Frontend (Open SaaS)** | 5/5 | 0/5 | 100% ✅ |
| **Fase 15: Backlog & Estabilización** | 0/3 | 3/3 | 0% 🔄 |

**Progreso Total**: 40/49 tareas completadas (81%)
...
*   **Tarea 9.1: Perímetro y Red (Checklist 1, 2)** ✅ (Ref: `EVIDENCE_Security_2`)
    *   **Cloudflare**: WAF, Rate Limiting, Bot Fight Mode.
    *   **SSL**: Full (Flex) + HSTS.
    *   **Hardening**: Nginx Proxy (Isolation), UFW, Allowlist Cloudflare IPs.
...
## 🏴 Fase 14: Frontend (Open SaaS) - ✅ Completado

**Objetivo**: Implementar Dashboard visual consumiendo la API de Securetag.
**Stack**: Wasp (React + Node.js + Prisma) + TailwindCSS.
**Base**: `frontend/open-saas-main/template/app`.

*   **Tarea 14.1: Análisis y Arquitectura Inicial** ✅ (Ref: `EVIDENCE_Frontend_1`, `EVIDENCE_Frontend_2`)
    *   Setup Docker y entorno Wasp. 
    *   Adaptación de marca (Cyberpunk Theme: `SplitAuthLayout`, `LetterGlitch`).
    *   **Navegación**: Sidebar contextual (`ProductSwitcher`).

*   **Tarea 14.2: Dashboard de Vulnerabilidades** ✅ (Ref: `EVIDENCE_Frontend_2`, `EVIDENCE_Frontend_3`)
    *   **BFF Pattern**: Endpoint `/dashboard/stats` para desacoplar DB.
    *   **Visualización**: Charts de Severidad (Donut Multi-segmento), Historial de Projectos.
    *   **Conexión Real**: Consumo de API Securetag con Proxy reverso (Nginx `Host` injection).

*   **Tarea 14.3: Gestión de Escaneos y Costos** ✅ (Ref: `EVIDENCE_Frontend_3`, `EVIDENCE_Frontend_5`)
    *   **Wizard**: File Uploader con validación de créditos (Single Source of Truth `sastCosts.ts`).
    *   **UI**: Feedback en tiempo real ("INSUFFICIENT FUNDS").
    *   **UX**: Páginas de error inmersivas (404/500 FuzzyText).

*   **Tarea 14.4: Identidad y Seguridad (Identity Linking)** ✅ (Ref: `EVIDENCE_Frontend_4`, `EVIDENCE_Frontend_7`)
    *   **Sincronización**: Wasp User <-> Securetag Core User (`auth-sync.ts`).
    *   **System Client**: Uso de `SECURETAG_SYSTEM_SECRET` para operaciones privilegiadas.
    *   **API Key Hardening**: Almacenamiento de solo hashes SHA-256 en ambas DBs.

*   **Tarea 14.5: Facturación y Monetización** ✅ (Ref: `EVIDENCE_Frontend_6`)
    *   **PayPal**: Integración de Créditos y Suscripciones (Webhooks `PAYMENT.SALE.COMPLETED`).
    *   **Sync**: Sincronización automática de saldo y plan con Backend Core.
    *   **Ledger**: Tabla `CreditUsage` para auditoría financiera.
