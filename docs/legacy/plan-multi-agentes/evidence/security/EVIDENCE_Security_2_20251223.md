...
## 🚑 10. Resolución de Incidente: Bucle de Redirecciones (ERR_TOO_MANY_REDIRECTS)

### 📍 Descripción del Problema
El usuario reportó que la página principal `securetag.com.mx` no cargaba en Safari, mostrando el error "Demasiados redireccionamientos", mientras que la API funcionaba correctamente.

### 🔍 Diagnóstico
Se identificó una configuración de **SSL Mixto** conflictiva:
1.  **API (`api` -> VPS)**: Requiere modo **Flexible** (Cloudflare -> HTTP -> Servidor), ya que el backend no tiene SSL configurado internamente.
2.  **Web Principal (`securetag` -> GoDaddy)**: El hosting de GoDaddy fuerza una redirección a HTTPS.
    *   Cloudflare (Flexible) conectaba por HTTP.
    *   GoDaddy respondía "301 Redirect to HTTPS".
    *   Cloudflare devolvía el redirect al navegador, creando un bucle infinito.

### ✅ Solución Implementada (Configuration Rules)
Se creó una **Regla de Configuración** en Cloudflare para aplicar modos SSL diferenciados por subdominio.

**Regla: `Web Principal SSL Full`**
*   **Trigger**: `Hostname equals securetag.com.mx` OR `Hostname equals www.securetag.com.mx`
*   **Acción**: Force **SSL/TLS encryption mode: Full**
*   **Resultado**: Cloudflare ahora se conecta vía HTTPS al hosting de GoDaddy (rompiendo el bucle), mientras mantiene el modo Flexible para la API.
