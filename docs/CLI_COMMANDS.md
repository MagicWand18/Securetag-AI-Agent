...
Referencia completa de comandos CLI de Securetag AI y capacidades del modo interactivo.
...
### `securetag-ai` (Default)
Start interactive session.

```bash
securetag-ai
securetag-ai i
securetag-ai interactive
securetag-ai interactive --mode redteam --model opus-4
```
...
```bash
securetag-ai flows                        # Interactive menu
securetag-ai flows --list                 # List all workflows
securetag-ai flows --category security    # Filter by category
securetag-ai flows --difficulty beginner  # Filter by difficulty
```
...
```bash
securetag-ai scan                                # Full desktop scan
securetag-ai scan --quick                        # Quick security check
securetag-ai scan --network                      # Network connections
securetag-ai scan --network --nmap --target 192.168.1.0/24  # Nmap scan
securetag-ai scan --json report.json --md report.md
```
...
### `securetag-ai webscan`
Web application vulnerability scanning.

```bash
securetag-ai webscan https://example.com
securetag-ai webscan --full https://myapp.local
securetag-ai webscan --ctf https://ctf.example.com
securetag-ai webscan https://example.com --nuclei --sslscan --sqlmap
securetag-ai webscan https://example.com --with-mcp  # Run all MCP tools
```
...
### `securetag-ai codeaudit`

Análisis estático de seguridad de código con Semgrep.

```bash
securetag-ai codeaudit <ruta>
securetag-ai codeaudit . --config auto
securetag-ai codeaudit ~/proyectos/app --severity high
securetag-ai codeaudit . --json --output semgrep.json
```

Opciones:
- `--config <cfg>`: `auto` o ruta a archivo de reglas de Semgrep.
- `--severity <level>`: `info|low|medium|high|critical` para filtrar mínimo.
- `--json`: salida JSON cruda de Semgrep.
- `--output <file>`: guarda resultados en archivo.

Notas:
- Requiere Semgrep instalado en el sistema. Verifica con `securetag-ai tools --check semgrep`.
- Resultados normalizados al formato de `SecurityFinding` para informes consistentes.

Ejemplos rápidos:
```bash
# Sin filtro: todas las severidades
securetag-ai codeaudit . --config auto
securetag-ai codeaudit . --config auto --severity all

# Solo altas/críticas
securetag-ai codeaudit . --severity high
securetag-ai codeaudit . --severity critical

# Solo medias
securetag-ai codeaudit src --severity medium
...
### `securetag-ai mobilescan`
**NEW!** Mobile app security scanning (Android APK / iOS IPA).

```bash
securetag-ai mobilescan app.apk
securetag-ai mobilescan --scan-type static app.apk
securetag-ai mobilescan --scan-type dynamic app.apk
securetag-ai mobilescan app.ipa --model opus-4
securetag-ai mobilescan app.apk --json report.json --md report.md
```
...
```bash
# Full reconnaissance
securetag-ai recon example.com --full
securetag-ai recon user@example.com --full

# Domain-focused scan
securetag-ai recon example.com --domain

# Person/username lookup
securetag-ai recon john_doe --person

# Individual tools
securetag-ai recon subdomains example.com
securetag-ai recon breach user@example.com
securetag-ai recon tech https://example.com
securetag-ai recon username john_doe
securetag-ai recon dns example.com
securetag-ai recon whois example.com
securetag-ai recon emails example.com
securetag-ai recon wayback example.com
securetag-ai recon ip 1.1.1.1
securetag-ai recon reverse-ip 1.1.1.1
```
...
```bash
securetag-ai pcap capture.pcap
securetag-ai pcap --mode full network-traffic.pcap
securetag-ai pcap --mode threat-hunt suspicious.pcap
securetag-ai pcap --filter tcp --port 443 traffic.pcap
securetag-ai pcap --extract-iocs --mitre capture.pcap
securetag-ai pcap suspicious.pcap --preserve-evidence --case-number CASE-2024-001
```
...
```bash
securetag-ai harden
securetag-ai harden --recommendations
securetag-ai harden --json hardening-report.json
```
...
securetag-ai auto "scan example.com for vulnerabilities"

# Red team mode
securetag-ai auto "find attack paths in staging.myapp.com" --mode redteam --verbose

# Blue team mode
securetag-ai auto "analyze logs for IOCs in /var/log/auth.log" --mode blueteam

# Web pentest mode
securetag-ai auto "audit WordPress site at blog.example.com" --mode webpentest

# OSINT mode
securetag-ai auto "gather intel on target-company.com" --mode osint

# Extended thinking for complex tasks
securetag-ai auto "comprehensive security assessment" --thinking --verbose

# Export execution context
securetag-ai auto "scan 192.168.1.0/24" --export scan-context.json
```
...
```bash
securetag-ai chat
securetag-ai chat --mode blueteam
securetag-ai chat --mode webpentest --model opus-4
```
...
Start interactive mode with `securetag-ai` or `securetag-ai interactive`.
...
```bash
# CLI mode
securetag-ai scan --model opus-4-1
securetag-ai webscan https://example.com --model gemini-2.5-flash
securetag-ai pcap traffic.pcap --model haiku-4
...
```bash
# Get started
securetag-ai                           # Interactive session (recommended)
securetag-ai flows                     # Browse workflows (beginner-friendly)

# Security scanning
securetag-ai scan                      # Desktop security scan
securetag-ai webscan https://example.com  # Web vulnerability scan
securetag-ai mobilescan app.apk        # Mobile app scan (NEW!)

# OSINT
securetag-ai recon example.com --full  # Full reconnaissance

# Analysis
securetag-ai pcap capture.pcap         # Network traffic analysis
...
```bash
securetag-ai --help                    # Show all commands
securetag-ai <command> --help          # Command-specific help
...
1. **Start with Workflows**: `securetag-ai flows` provides guided experiences
