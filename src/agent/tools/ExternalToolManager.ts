...
export class ExternalToolManager {
  ...
  private static async checkTool(
    ...
    try {
      ...
      // Check local project tools bin directory
      const localBin = path.join(process.cwd(), '.securetag', 'tools', 'bin');
      const candidates = [
        path.join(localBin, toolDef.command),
        path.join(localBin, `${toolDef.command}.sh`),
      ];
  ...
  static async execute(
    ...
    try {
      const binary = tool.path || tool.command;
  ...
  static async generateInstallGuide(): Promise<string> {
    ...
    lines.push('The following external tools are not installed but can enhance Securetag AI:\n');
    ...
    lines.push('\n💡 Tip: These tools are optional. Securetag AI works without them using built-in capabilities.');
  ...
  static async generateStatusReport(): Promise<string> {
    ...
    if (unavailable.length > 0) {
      ...
      lines.push('');
      lines.push('💡 Run `securetag-ai tools --install-guide` for installation instructions');
    }
